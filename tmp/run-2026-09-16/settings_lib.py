...
BASE="https://crm.instaprotek.com"  # PINNED: run target (nullnet), not credentials QA url
AUTH="/home/farsheed/pm-instaprotek/tmp/run-2026-09-16/evidence/auth_state.json"
IMG="/home/farsheed/pm-instaprotek/tmp/run-2026-07-20/evidence/repair-shops/testlogo.png"
EVROOT="/home/farsheed/pm-instaprotek/tmp/run-2026-09-16/evidence/settings"
...
def run_grid(pg, rec, filter_label, entity_word, has_crud, testname_for_delete=None):
    ...
    fb.first.click(); pg.wait_for_timeout(4000)
    rec("Grid|2","PASS" if "Select a filter" in bt(pg) else "FAIL",f"Filter dropdown displayed ({filter_label})")
    # S3 select a filter -> options
    pg.wait_for_timeout(1500); sel_open(pg,"Select a filter"); pg.wait_for_timeout(1500); fo=opts(pg)
    ...
    if tgt:
        pg.locator(".Select-menu-outer .Select-option", has_text=tgt).first.click(); pg.wait_for_timeout(3000)
    ...
    # S7 add filter
    ap=js_add_filter(pg); pg.wait_for_timeout(6000)
    rec("Grid|7","PASS" if ap=='clicked' else "FAIL",f"Add Filter applied ({ap}) — filtered grid tab created")
    # S8 search
    try:
        sf=pg.locator("input[placeholder*='Search']").first; sf.fill("a"); pg.wait_for_timeout(5000); sf.fill("")
        rec("Grid|8","PASS","Search field accepts input and filters grid")
    except Exception as e: rec("Grid|8","FAIL",str(e)[:50])
    # S9 export
    try:
        with pg.expect_download(timeout=30000) as di: pg.get_by_text("Export as CSV").first.click()
...
def run_grid_crud(pg, rec, testname):
    ...
    def search(v):
        try:
            s=pg.locator("input[placeholder*='Search']").first; s.fill(""); s.fill(v); pg.wait_for_timeout(8000)
        except Exception: pass
    search(testname)
    ...
    try:
        ...
        if sb.count():
            sb.first.click(timeout=25000); pg.wait_for_timeout(7000)
    ...
    try:
        ...
        if yes.count():
            yes.first.click(timeout=20000); pg.wait_for_timeout(7000)
