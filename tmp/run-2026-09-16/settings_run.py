...
def main(key):
    ...
    with sync_playwright() as p:
        ...
        pg.goto(BASE+cf["route"], wait_until="domcontentloaded", timeout=90000)
        assert slib.wait_grid(pg, cf["filt"]), "grid nav failed"

        # ---- NEW <entity> (create test record first) ----
        print(f"=== NEW ({cf['new_sec']}) ===", flush=True)
        pg.get_by_text("addNew").first.click(); pg.wait_for_timeout(9000)
        ...
        if cf["new_steps"]=="name":
            ...
            pg.locator(".md-dialog button").filter(has_text="Save").first.click(); pg.wait_for_timeout(9000)
        ...
        pg.goto(BASE+cf["route"], wait_until="domcontentloaded", timeout=90000); slib.wait_grid(pg, cf["filt"])
