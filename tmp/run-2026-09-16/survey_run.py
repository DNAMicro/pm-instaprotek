...
Q="RegressionTest0916 survey question?"
Q2="RegressionTest0916 survey question edited?"
R={}
...
with sync_playwright() as p:
    ...
    # Grid|2 edit
    info=find_actions(pg, "RegressionTest0916")
    e=click_action(pg,"RegressionTest0916","edit"); pg.wait_for_timeout(5500)
    ...
    d=click_action(pg,"RegressionTest0916","delete"); pg.wait_for_timeout(3500)
    ...
    gone=False
    if yn:
        click_action(pg,"RegressionTest0916","delete"); pg.wait_for_timeout(3500)
        pg.evaluate(f"""()=>{{const ds=[...document.querySelectorAll('.md-dialog')].filter(x=>!x.classList.contains('md-dialog--full-page'));
          const d=ds[ds.length-1];const b=[...d.querySelectorAll('button')].find(x=>/Yes/.test(x.textContent));if(b)b.click();}}""")
        pg.wait_for_timeout(7000)
        pg.goto(N.BASE+"/portal/survey", wait_until="domcontentloaded", timeout=60000); pg.wait_for_timeout(8000)
        gone = "RegressionTest0916" not in N.bt(pg)
