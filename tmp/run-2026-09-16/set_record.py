...
NOTE="RegressionTest Note Sep16"
...
def teardown(pg, rec_url, route, label):
    """Delete the entity we created — ONLY after proving the open record is ours.

    2026-09-16: without this guard a teardown deleted a live production plan.
    Ownership is proven from the open record's own text, never from a grid row count."""
    try:
        pg.goto(rec_url, wait_until="domcontentloaded", timeout=60000); pg.wait_for_timeout(12000)
        try:
            guard.assert_ours(pg, f"teardown delete of {label}")
        except guard.NotOurs as e:
            print(f"  [teardown] {label}: ABORTED — {e}", flush=True)
            return False
